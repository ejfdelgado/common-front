...
@Component({
  ...
  templateUrl: './index-pug.html',
  styleUrl: './index-pug.scss',
})
export class IndexPug extends AuthenticatedComponent implements OnInit, OnDestroy {
  ...
  async deleteDocument({ model }: { model: any }) {
    ...
    const index = this.pugs.indexOf(model);
    if (index >= 0) {
      this.pugs.splice(index, 1);
      this.cdr.detectChanges();
    }
  }
  ...
  async pageDocuments(startover: boolean = false) {
    ...
    try {
      if (startover && this.pugs.length > 0) {
        this.pugs.splice(0, this.pugs.length);
      }
      ...
      if (!startover) {
        if (this.pugs.length > 0) {
          pagingOptions.lastDoc = this.pugs[this.pugs.length - 1];
        }
      }
      const page = (await this.firestoreSrv.paging(pagingOptions));
      this.pugs.push(...(page as PugDataType[]));
      this.cdr.detectChanges();
    } catch (err) {
