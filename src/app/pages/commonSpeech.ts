...
export class CommonSpeech extends CommonComponent {
  ...
  async genericVoiceQueryInternal(
    query1: string[],
    options: VoiceQuery[],
    timeout: number | null = 5000,
    repeat: boolean = true,
    ...
    const promise = new Promise<VoiceAnswer>((resolve, reject) => {
      ...
      subscription = word$.pipe(debounceTime(300)).subscribe(addWordFun);
    });
    promise.finally(() => {
      if (timeoutHandler) {
        ...
        promise
          ...
          .catch(() => {
            requestAnimationFrame(async () => {
              const response = await this.genericVoiceQueryInternal(
                query1,
                options,
                timeout,
                repeat,
  ...
  async askName() {
    ...
    do {
      nombre = await this.genericVoiceQuery(
        ...
        [
          { reg: /(yo\s)(me\s)(llamo\s)(.+)$/gi, index: 4 },
          { reg: /(me\s)(llamo\s)(.+)$/gi, index: 3 },
          { reg: /(llamame\s|llameme\s)(.+)$/gi, index: 2 },
          { reg: /(me\s)(puedes?\s)(llamar\s)(.+)$/gi, index: 4 },
          { reg: /(mi\s)(nombre\s)(es\s)(.+)$/gi, index: 4 },
          { reg: /(.+)/gi, index: 1 },
        ],
      );
      ...
      const confirmacion = await this.genericVoiceQuery(
        ...
        [
          { reg: /(no|incorrecto|mal)/gi, index: 1 },
          { reg: /(si|correcto|bien|confirmado)/gi, index: 1 },
        ],
      );
